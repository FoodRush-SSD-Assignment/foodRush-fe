...
const ViewRestaurants = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status?.toLowerCase()) {
      case "pending":
        return <span className="w-2.5 h-2.5 bg-blue-500 inline-block rounded-full mr-2"></span>;
      case "approved":
        return <span className="w-2.5 h-2.5 bg-emerald-500 inline-block rounded-full mr-2"></span>;
      case "suspended":
        return <span className="w-2.5 h-2.5 bg-gray-400 inline-block rounded-full mr-2"></span>;
      default:
        return null;
    }
  };
    ...
    >
      ...
      <div className="p-5">
        <div className="flex justify-between items-start mb-3">
          ...
          <div className={`px-3 py-1 rounded-full text-xs font-medium ${getStatusClass(restaurant.status)} border flex items-center`}>
        ...
        <div className="space-y-3 text-sm text-gray-600">
          <div className="flex items-start">
            ...
            <span className="leading-tight">{restaurant.location || "Location not specified"}</span>
          </div>
          <div className="flex items-start">
            <svg className="h-5 w-5 mr-2 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
            </svg>
            <span className="leading-tight">{restaurant.contactNumber || "No contact number"}</span>
          </div>
          <div className="flex items-start">
            <svg className="h-5 w-5 mr-2 text-gray-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
            </svg>
            <span className="leading-tight capitalize">{restaurant.category?.replace("_", " ") || "Uncategorized"}</span>
          </div>
        </div>
  ...
  const groupedRestaurants = {
    ...
    suspended: restaurants.filter(r => r.status?.toLowerCase() === "suspended"),
  };
  ...
  return (
    ...
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-secondary">My Restaurants</h1>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-10 text-center max-w-2xl mx-auto shadow-sm">
          ...
          <p className="text-gray-500 mb-6 max-w-md mx-auto">You haven't added any restaurants yet. Get started by adding your first restaurant.</p>
          <button
            onClick={() => navigate(`/merchant/add-restaurant/${ownerId}`)}
              ...
              <div key={status} className="mb-12">
                <div className="flex items-center mb-5">
                  <h2 className="text-xl font-semibold text-gray-800 capitalize">
                    {status === "approved" ? "Active" : status === "pending" ? "Pending Approval" : "Suspended"} Restaurants
                  </h2>
