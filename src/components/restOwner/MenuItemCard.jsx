...
const MenuItemCard = ({ id }) => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await restaurantApi.put(`/items/${selectedItem._id}`, formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <p className="ml-4 text-lg font-medium text-secondary">Loading menu items...</p>
      </div>
    );
  }
          ...
          >
            ...
            {item.imageUrl ? (
              <div className="relative h-56 overflow-hidden">
                <img src={item.imageUrl} alt={item.itemName} className="w-full h-full object-cover transition-transform duration-300 hover:scale-105" />
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-2">
                <h3 className="text-xl font-bold text-secondary">{item.itemName}</h3>
                <span className="text-lg font-bold text-primary">Rs. {item.itemPrice}</span>
              </div>
              
              <p className="text-gray-600 mb-4 line-clamp-2">{item.itemDescription}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60">
          <div className="bg-white rounded-lg p-8 w-full max-w-md shadow-2xl transform transition-all">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-secondary">Edit Menu Item</h2>
              <button 
                ...
                <label className="inline-flex items-center cursor-pointer">
                  <div className="relative">
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`block w-14 h-8 rounded-full transition-colors ${formData.isAvailable ? 'bg-green-500' : 'bg-gray-300'}`}></div>
                    <div className={`absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition-transform transform ${formData.isAvailable ? 'translate-x-6' : ''}`}></div>
                  </div>
