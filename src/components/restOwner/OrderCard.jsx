...
function OrderCard({ order }) {
  ...
  const getStatusBadgeClass = (statusValue) => {
    ...
    return statusMap[statusValue] || 'bg-gray-100 text-gray-800 border-gray-200';
  };

  return (
    <div className="overflow-x-auto">
      {alert.show && (
        <div className={`${alert.type === 'success' ? 'bg-green-50 border-green-400' : 'bg-red-50 border-red-400'} border-l-4 p-4 fixed top-4 right-4 z-50 shadow-lg rounded-md`}>
          <div className="flex">
            ...
            <div className="ml-3">
              <h3 className={`text-sm font-medium ${alert.type === 'success' ? 'text-green-800' : 'text-red-800'}`}>
                {alert.title}
              </h3>
              <div className={`mt-2 text-sm ${alert.type === 'success' ? 'text-green-700' : 'text-red-700'}`}>
                <p>{alert.message}</p>
              </div>
            </div>
            <div className="ml-auto pl-3">
              <div className="-mx-1.5 -my-1.5">
                <button
                  ...
                  className={`inline-flex ${alert.type === 'success' ? 'bg-green-50 text-green-500 hover:bg-green-100 focus:ring-green-600' : 'bg-red-50 text-red-500 hover:bg-red-100 focus:ring-red-600'} rounded-md p-1.5 focus:outline-none focus:ring-2 focus:ring-offset-2`}
                >
                  <X className="h-5 w-5" />
                </button>
              </div>
      ...
      <div className="border border-gray-200 rounded-lg shadow-md overflow-hidden mb-6">
        <div className="bg-white p-4 flex justify-between items-center border-b border-gray-200">
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className={`inline-flex px-3 py-1 text-xs font-medium rounded-full ${getStatusBadgeClass(status)}`}>
        ...
        <table className="min-w-full bg-white">
          ...
          <tbody className="divide-y divide-gray-200">
            <tr>
              ...
              <td className="py-3 px-4">
                <ul className="text-sm text-gray-700 space-y-2">
                  {order.items.map((item, index) => (
                    <li key={index} className="flex justify-between items-center border-b border-gray-100 pb-1 last:border-b-0 last:pb-0">
                ...
                <div className="mt-2 pt-2 border-t border-gray-100 text-right">
                  <span className="text-sm font-bold text-gray-900">Total: Rs.{order.totalPrice}</span>
                </div>
              </td>
              ...
              <td className="py-3 px-4">
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    <option value="cancelled_by_restaurant">Cancelled by Restaurant</option>
                  </select>
                  {loading && (
              ...
              {order.deliveryPersonId && (
                <td className="py-3 px-4">
                  <div className="text-sm font-medium text-gray-900">{order.deliveryPersonName}</div>
                </td>
              )}
              ...
              {order.estimatedDeliveryTime && (
                <td className="py-3 px-4">
                  <div className="text-sm text-gray-700">{formatDate(order.estimatedDeliveryTime)}</div>
                </td>
              )}
            </tr>
          </tbody>
