...
const MapLocation = () => {
  ...
  const fetchDriverDetails = async () => {
    ...
    try {
      ...
      displayError(
        "driver",
        err.response?.data?.error || "Failed to load driver data"
      );
    }
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fetchLocations = async () => {
        try {
          const restaurantLoc = await getLatLngFromAddress(
            parsedOrder.restaurantLocation,
            "restaurant"
          );
          ...
          const deliveryLoc = await getLatLngFromAddress(
            ...
            <div className="mb-6">
              {Object.entries(errors).map(([type, message]) =>
                message ? (
                  <div
                    key={type}
                    className="p-3 mb-2 bg-red-100 border-l-4 border-red-500 text-red-700 flex justify-between items-center"
                  >
                    <div>
                    ...
                    {errors.order ? (
                      ...
                      <>
                        ...
                        <p className="text-gray-700">
                          ...
                          {currentOrder.customerName || "Anonymous"}
                        </p>
                      </>
                    )}
                  </div>
                        ...
                        <Popup>
                          <div className="text-center">
                            ...
                            <p className="text-sm text-gray-600">
                              Driver:{" "}
                              {driver.name || userData?.firstname || "You"}
                            </p>
                          </div>
                        </Popup>
                      </Marker>
                      ...
                      >
                        <Popup>
                          <div className="text-center">
                            ...
                            <p className="text-sm text-gray-600">
                              Phone: {currentOrder?.customerMobileNo || "N/A"}
                            </p>
                          </div>
                        </Popup>
                      </Marker>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      navigate("/orderstatus", {
                        state: { orderId: currentOrder.orderId },
                      });
                    }}
                  >
                    Accept order
